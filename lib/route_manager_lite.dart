...
class RouteManagerLite {
  ...
  List<(int, String, String)> updateStatesOfSidePoints(
    ...
    if (currentLocationIndex < 0 || currentLocationIndex >= _route.length) {
      ...
      for (final int i in sidePointIndexes) {
        ...
        newSidePointsData.add((i, data.$2, data.$3));
      }
  ...
  List<(int, String, String)> updateNStatesOfSidePoints(
      LatLng currentLocation,
      {int amountOfUpdatingSidePoints = 40}) {
    updateListeners();
  ...
  Map<int, (int, String, String)> get sidePointsStatesHashTable => _sidePointsStatesHashTable;
