...
class RouteManager {
    ...
    if (route.isEmpty) {
      ...
      for (int i = 0; i < (route.length - 1); i++) {
        ...
        final double distance =
            _getDistance(point1: route[i], point2: route[i + 1]);
  ...
  final FileLogHandler logHandler = FileLogHandler(
    'path_passing_imitation.txt',
    'obj_errors.txt',
  );
  ...
  double _getAngleBetweenVectors({
    ...
    final double angle =
        math.acos((dotProduct / (magnitudeV1 * magnitudeV2)).clamp(-1, 1)) *
            (180 / math.pi);
    return angle;
  }
  ...
  int _findClosestWayPointV2({required LatLng currentLocation}) {
    ...
    for (final int keyIndex in keys) {
      ...
      final double angle = _getAngleBetweenVectors(v1: motionVector, v2: routeVector);
      if (angle <= 46) {
  ...
  List<(int, String, String)> updateCurrentLocation({
    ...
    final String log = '{\n'
        ...
        '},';
    logHandler.pathImitationLogger(log);
    ...
    if (currentLocationIndex < 0 || currentLocationIndex >= _route.length) {
      logHandler.objErrorsLogger(
          'RouteManager got a currentLocationIndex equal to 0.\n${toString()}'
          '   researchFuncVersion: $researchFuncVersion\n'
          '   previousCurrentLocation: $_previousCurrentLocation\n'
          '   previousCurrentLocationIndex: $_previousCurrentLocationIndex\n');
  ...
  (double, bool, int) traveledRoute({required LatLng currentLocation}) {
    ...
    final double distance =
        _distanceTraveledBySegmentIndex[_previousCurrentLocationIndex]!;
    //length of travelled distance in that segment
    final double distanceToNextRoutePoint =
        _getDistance(point1: currentLocation, point2: _nextRoutePoint);

    final double delta = _segmentLength[_previousCurrentLocationIndex]! -
        distanceToNextRoutePoint;
    final double additionalDistance = delta < 0 ? 0 : delta;

    final double passedRouteLength = distance + additionalDistance;

    final bool isFinish =
        (_routeLength - passedRouteLength) < _distanceToTheFinish;
  ...
  double getDistanceToWayPointInWay(
      {required LatLng currentLocation, required int finish}) {
  ...
  String toString() {
    return 'INFO about instance:\n'
        '   route: $_route\n'
        '   alignedSidePoints: $_alignedSidePoints\n'
        '   laneWidth: $_laneWidth\n'
        '   laneExtension: $_laneExtension\n';
  }
}
